...
public class DetermineBasalResultARG extends APSResult {
    private static final Logger log = LoggerFactory.getLogger(L.APS);

    private double eventualBG;
    private double snoozeBG;

    DetermineBasalResultARG(JSONObject result) {
        this();
        date = DateUtil.now();
        json = result;
        try {
    ...
    private DetermineBasalResultARG() {
        hasPredictions = true;
    }

    @Override
    public DetermineBasalResultARG clone() {
        DetermineBasalResultARG newResult = new DetermineBasalResultARG();
        doClone(newResult);
